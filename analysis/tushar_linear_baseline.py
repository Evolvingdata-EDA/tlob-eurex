"""Tushar's linear next-tick model (L1-computable subset) vs TLOB, on OUR target.

Replicates his feature families using HIS exact ewma_vec, on our data, then fits
a multinomial logistic regression on the SAME target/split TLOB used:
  target  = SMOOTHED_MID_TIME, h=1s, label_threshold_ticks=0.0 (pure-sign drift)
...
def main() -> None:
    # ---- 1. target frame + label (TLOB-identical) -------------------------- #
    dl = DataLoader(
    ...
    lab = labeling(g, 1, cst.LabelingType.SMOOTHED_MID_TIME, 3, ["FGBL_TBBO"], 0.01, 10,
    ...
    X.to_parquet("results/tushar_linear_features.parquet", index=False)
    ...
    print(f"\n[linear baseline] train {len(ytr):,} / test {len(yte):,}   TLOB ref: acc 0.611 / macroF1 0.609")
    print(f"{'features':>30s} {'acc':>7s} {'macroF1':>8s} {'upF1':>6s} {'flatF1':>7s} {'downF1':>7s}")
    for name, cols in variants.items():
        ...
        per = f1_score(yte, pred, labels=[0, 1, 2], average=None)
        mac = f1_score(yte, pred, labels=[0, 1, 2], average="macro")
        print(f"{name:>30s} {acc:>7.4f} {mac:>8.4f} {per[0]:>6.3f} {per[1]:>7.3f} {per[2]:>7.3f}")
